...
+ (void)requestWithPath:(NSString *)path WithJson:(NSDictionary *)parameters block:(void(^)(id JSON, NSError *error))block{
    ...
    */
    ...
    NSLog(@"path == %@",path);
    [manager POST:[NSString stringWithFormat:@"%@%@",mainURL,path] parameters:parameters progress:^(NSProgress * _Nonnull uploadProgress) {
...
+ (void)imageRequestWithURL:(NSString *)path WithJson:(NSDictionary *)parameters formDate:(void(^)(id< AFMultipartFormData> formData))formDa  block:(void(^)(id JSON, NSError *error))block{
    ...
    */
    ...
    [manager POST:[NSString stringWithFormat:@"%@%@",mainURL,path] parameters:parameters constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
        if (formDa) {
            formDa(formData);
        }
...
+ (void)getRequestWithURL:(NSString *)url block:(void(^)(id JSON,NSError *error))block{
    ...
    [manager.requestSerializer setValue:identifierForVendor forHTTPHeaderField:@"token"];
    */
    [manager GET:[NSString stringWithFormat:@"%@%@",mainURL,url] parameters:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
